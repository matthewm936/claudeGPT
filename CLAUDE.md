...
## Filing — Dual Agent Architecture

**Your only job is the conversation.** Filing is handled by a background agent you spawn — it never competes with your thinking.

On every substantive message, spawn a filing agent (using the Agent tool, `run_in_background: true`) with:
1. **What the user said** — their full input, verbatim or close to it
2. **Conversation context** — any prior messages needed to understand the input (e.g., what you said that they're responding to)
3. **Today's date** — for file naming
4. **KB root path** — the profiles directory

The filing agent's instructions live in `.claude/filing-agent.md`. You don't need to repeat the full prompt — describe the task, pass the user's input, and include enough context for the agent to understand what was shared. The agent handles routing, synthesis, changelog, and reporting.

**When not to spawn:** Casual messages (greetings, banter, quick factual questions). If the user didn't share anything about their life, there's nothing to file.

**When the agent completes**, append its report to your next response as:
...
If the agent hasn't finished by the time you respond, note "KB filing in progress" and report changes in your next message. Keep the display human-readable — no file paths, no code formatting.
